...
// constructor initlializing the board with random boolean values, the board dimensions are given as parameters
    public GameOfLifeBoard(int m, int n) {
        ...
        //method adding the missing cells around the main board necessary for conting the amount of alive cells around the specific one
        private void wrap(){
        int m = board.length;
       ...
       board[m-1][n-1]=board[1][1];
        ...
        //we take into action only the inner part of the table, we skip the outer "frame" of cells, so we starting from second index (1), and we end on index equal to size of the temp. table ( we run through size of the temp. board, which repersents only the "main" one)
        for(int i=1;i<= temporary.length;i++){
            for(int j=1;j<=temporary[0].length;j++){
                ...
                //if the cell is alive and has 2 or 3 neighbours also alive, then stays alive, otherway it dies (due to overwhelming or starving)
                if(board[i][j]) {
                    switch (--alive) {
                        case (2):
                        case (3):
